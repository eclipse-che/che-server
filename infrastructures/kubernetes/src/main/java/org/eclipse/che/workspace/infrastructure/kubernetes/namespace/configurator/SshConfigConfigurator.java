...
public class SshConfigConfigurator implements NamespaceConfigurator {
  ...
  public void configure(NamespaceResolutionContext namespaceResolutionContext, String namespaceName)
    ...
    String configMapName = "git-ssh-config";
    ...
    if (secret != null && secret.getData().containsKey(configKeyName)) {
      secret.getData().remove(configKeyName);
      client.secrets().inNamespace(namespaceName).resource(secret).patch();
    }

    ConfigMap sshConfigConfigMap =
        client.configMaps().inNamespace(namespaceName).withName(configMapName).get();
    if (sshConfigConfigMap == null) {
          ...
          new ConfigMapBuilder()
              .withMetadata(
                  new ObjectMetaBuilder()
                      ...
                      .withLabels(labels)
                      .build())
              .withData(Collections.singletonMap("ssh_config", config))
              .build();
